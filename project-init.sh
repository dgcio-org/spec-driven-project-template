#!/bin/bash
# Spec-Driven Project Template �?项目初始化脚�?
#
# 用法:
...
TEMPLATE_REPO="https://github.com/dgcio-org/spec-driven-project-template.git"
SCAFFOLD_REPO="https://github.com/LucasGuoo/agent-scaffold.git"

echo "=== 初始化项�? $PROJECT_NAME ==="
...
if command -v git &>/dev/null; then
    ...
    echo "  �?未检测到 git，跳�?agent-scaffold 安装"
fi

# 清理模板内置 skills 目录（已�?agent-scaffold 接管�?
if [ -d "skills" ]; then
    rm -rf skills
    echo "  �?已清理内�?skills 目录（由 agent-scaffold 接管�?
fi
...
echo ""
echo "=== 项目初始化完�? $TARGET_DIR ==="
echo ""
echo "下一�?"
echo "  1. 编辑 docs/project-rules.md，填写项目特定规�?
echo "  2. 编辑 docs/constitution.md，填写工具链约束"
echo "  3. 编辑 AGENTS.md，填写技术栈，调整工具兼容性矩�?
echo ""
echo "  agent-scaffold skills 已安装。Agent 启动后自动识别并加载�?
echo ""
echo "  GitHub 仓库创建:"
echo "    gh repo create dgcio-org/$PROJECT_NAME --private"
echo "    git remote add origin git@github.com:dgcio-org/$PROJECT_NAME.git"
